...
const FindAClass: React.FC<Props> = ({ userPicture, client_id }) => {
  ...
  useEffect(() => {
    var options = {
      ...
      rootMargin: '200px 0px -650px 0px',
      threshold: [0]
    };
  ...
  useEffect(() => {
    if(swiper && Number(format(chosenDay, 'e')) === 1) {
      swiper.slideTo(chosenDayIndex);
    }
    if(swiper && Number(format(chosenDay, 'e')) === 7) {
  ...
  return (
    <IonPage>
      <IonHeader style={{position: 'relative', top: '0px', zIndex: 20}} className='header' id='header'>
        <BasicHeader title={t('headers.find_a_class')} onClick={clearFilters} />
        <div>
      ...
      <IonContent>
        <div style={{position: 'relative', background: '#F4F4F4', minHeight: '100vh'}} id='body'>
          {!showLoading && (showBooked ?
            (Object.values(bookedOccurencesByDates).find((val) => {return val.length > 0;}) ? 
            <div style={{paddingTop: '200px', background: '#F4F4F4'}} id='body'>
              <div>
              {Object.keys(bookedOccurencesByDates).map((key, i) => {
                if(bookedOccurencesByDates[key].length){
                  ...
                  return (
                    <div style={{paddingBottom: '5px'}} id={format(new Date(key), 'eee') + ', ' + format(new Date(key), 'MMM')  + ' ' + format(new Date(key), 'd')} key={format(new Date(key), 'eee') + ', ' + format(new Date(key), 'MMM')  + ' ' + format(new Date(key), 'd')} ref={el => itemsRef.current[i] = el}>
                      <div className='sticky-date' style={{position: 'sticky', top: '170px', zIndex: 10, background: '#F4F4F4', paddingTop: '15px', paddingBottom: '15px'}} id={key + '\\' + index} >{/*'stickyDate_' + format(new Date(key), 'MMM') + '_' + format(new Date(key), 'd')*/}
             ...
             :
             <div style={{paddingTop: '200px'}} id='body'><div style={{paddingTop: '70px'}}><NoClassesPlaceholder src='/assets/icon/no_filtered_classes_image.svg' title={t('no_placeholder.no_classes_found')} subtitle={t('no_placeholder.no_classes_different_filter')}  /></div></div>)
             :
            (Object.values(occurencesByDates).find((val) => {return val.length > 0;}) ? 
            <div style={{paddingTop: '200px', background: '#F4F4F4'}} id='body'>
              {Object.keys(occurencesByDates).map((key, i) => {
